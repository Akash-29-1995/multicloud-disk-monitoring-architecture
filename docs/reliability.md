...
| **CloudWatch Agent down** | — for that host | Telemetry for that host | `agent-missing` alarm → `configure-monitoring.yml` |
...
Alarm (agent-missing) or failed validate
        │
        ▼
Re-run configure-monitoring.yml / enroll.yml  (idempotent)
        │
        ├── retries + delay + until on install/service
        ├── wait for IAM propagation (docs)
        └── validate_live.sh polls CloudWatch with timeout
        │
        ▼
...
| Metric appearance | `validate_live.sh` poll loop (default 300s) |
